...
class AppSettingsRepository @Inject constructor(
    ...
    private val prefsFlow = context.safeSettingsData

    /** Hiển thị ngày âm lịch trên ô calendar hay không */
    val lunarBadgeEnabled: Flow<Boolean> =
        prefsFlow.map { it[SettingsKeys.LUNAR_BADGE] ?: true }

    /** Thông báo nhắc nhở có bật không */
    val notifyEnabled: Flow<Boolean> =
        prefsFlow.map { it[SettingsKeys.NOTIFY_ENABLED] ?: true }

    /** Thông báo giờ đại cát có bật không */
    val gioDaiCatEnabled: Flow<Boolean> =
        prefsFlow.map { it[SettingsKeys.GIO_DAI_CAT] ?: false }

    /** Chế độ giao diện: "light", "dark", "system" */
    val themeMode: Flow<String> =
        prefsFlow.map { it[SettingsKeys.THEME_MODE] ?: "system" }

    /** Hiển thị ngày lễ / sự kiện */
    val festivalEnabled: Flow<Boolean> =
        prefsFlow.map { it[SettingsKeys.FESTIVAL_ENABLED] ?: true }

    /** Hiển thị câu danh ngôn mỗi ngày */
    val quoteEnabled: Flow<Boolean> =
        prefsFlow.map { it[SettingsKeys.QUOTE_ENABLED] ?: true }

    /** Nhắc nhở ngày lễ */
    val festivalReminderEnabled: Flow<Boolean> =
        prefsFlow.map { it[SettingsKeys.FESTIVAL_REMINDER] ?: true }

    /** Đơn vị nhiệt độ (°C / °F) */
    val tempUnit: Flow<String> =
        prefsFlow.map { it[SettingsKeys.TEMP_UNIT] ?: "°C" }

    /** Tên vị trí */
    val locationName: Flow<String> =
        prefsFlow.map { it[SettingsKeys.LOCATION_NAME] ?: "Hà Nội" }

    /** Giờ nhắc nhở */
    val reminderHour: Flow<Int> =
        prefsFlow.map { it[SettingsKeys.REMINDER_HOUR] ?: 7 }

    /** Phút nhắc nhở */
    val reminderMinute: Flow<Int> =
        prefsFlow.map { it[SettingsKeys.REMINDER_MINUTE] ?: 0 }
