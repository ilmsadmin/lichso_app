...
import { ArrowLeft, Clock, Eye, Newspaper, Sparkles, FolderOpen } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { usePublicCategoryBySlug } from "@/hooks/usePublicContent";
...
export default function CategoryArticlesClient({ slug }: { slug: string }) {
  const [page, setPage] = useState(1);
  const { data: categoryData, isLoading: categoryLoading } = usePublicCategoryBySlug(slug);
  const category = categoryData?.data;

  // Only fetch articles once we have the category's ID
  const { data: articlesData, isLoading: articlesLoading } = useQuery({
    queryKey: ["public-articles", { page, limit: 12, category_id: category?.id }],
    queryFn: () =>
  ...
  const CatIcon = category ? getCategoryIcon(category.slug) : FolderOpen;
  ...
  if (!categoryLoading && !category) {
    return (
      <div className="mx-auto max-w-[800px] px-4 py-16 text-center">
        ...
        <h1 className="text-text-dark mb-2 text-2xl font-semibold">Không tìm thấy danh mục</h1>
        <p className="text-text-soft mb-6">Danh mục bạn tìm kiếm không tồn tại.</p>
        <Button asChild>
    ...
    <div className="mx-auto max-w-[1180px] px-4 py-6 sm:px-6 sm:py-8 lg:px-7">
      ...
      <div className="mb-6 sm:mb-10 sm:text-center">
        {categoryLoading ? (
          ...
          <>
            ...
            <h1 className="text-text-dark mb-2 text-2xl font-[var(--font-lora)] font-bold sm:text-4xl">
              {category?.name}
            </h1>
            {category?.description && (
              <p className="text-text-soft mx-auto max-w-lg text-sm sm:text-base">
                {category.description}
              </p>
            )}
          </>
      ...
      {meta && !isLoading && (
        <div className="text-text-muted-ls mb-6 text-sm">
          <strong className="text-text-dark">{meta.total}</strong> bài viết trong danh mục này
        </div>
      )}
