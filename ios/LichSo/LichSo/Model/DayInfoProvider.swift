...
// ═══════════════════════════════════════════
// DayInfoProvider — Port chính xác từ Android
// ═══════════════════════════════════════════
...
class DayInfoProvider {
    ...
    func getDayInfo(dd: Int, mm: Int, yy: Int) -> DayInfo {
        ...
        return DayInfo(
            ...
            activities: DayActivitiesInfo(
                nenLam: activities.nenLam,
                khongNen: activities.khongNen,
                isXauDay: activities.isXauDay,
                isNguyetKy: activities.isNguyetKy,
                isTamNuong: activities.isTamNuong
            ),
            ...
            trucNgay: TrucNgayDisplayInfo(name: trucNgay.name, rating: trucNgay.rating),
            saoChieu: SaoChieuDisplayInfo(name: saoChieu.name, rating: saoChieu.rating),
            dayRating: dayRating,
    ...
    func getCalendarDays(year: Int, month: Int, weekStartSunday: Bool = false) -> [CalendarDay] {
        ...
        let offset: Int
        if weekStartSunday {
            offset = weekday - 1 // Sun=0, Mon=1..Sat=6
        } else {
        ...
        // Previous month
        guard let prevMonthDate = calendar.date(byAdding: .month, value: -1, to: firstDay),
        ...
        for i in stride(from: offset - 1, through: 0, by: -1) {
            ...
            let lunar = LunarCalendarUtil.convertSolar2Lunar(dd: d, mm: prevMonth, yy: prevYear)
            let lunStr = lunar.lunarDay == 1 ? "\(lunar.lunarDay)/\(lunar.lunarMonth)" : "\(lunar.lunarDay)"
            let dow = dayOfWeekFor(day: d, month: prevMonth, year: prevYear)
            days.append(CalendarDay(
                ...
                isSunday: dow == 7, isSaturday: dow == 6,
        ...
        for d in 1...daysInMonth {
            ...
            let lunStr = lunar.lunarDay == 1 ? "\(lunar.lunarDay)/\(lunar.lunarMonth)" : "\(lunar.lunarDay)"
            let dow = dayOfWeekFor(day: d, month: month, year: year)
            ...
            let saoChieu = SaoChieuCalculator.getSaoChieu(jd: jd)
            let activities = DayActivityCalculator.getDayActivities(jd: jd, lunarDay: lunar.lunarDay, lunarMonth: lunar.lunarMonth)
            let rating = calculateDayRating(truc: trucNgay, sao: saoChieu, activities: activities)
            ...
            days.append(CalendarDay(
                ...
                hasEvent: hasEvent, lunarDisplayText: lunStr,
        ...
        guard let nextMonthDate = calendar.date(byAdding: .month, value: 1, to: firstDay) else { return days }
        ...
        for d in 1...max(remaining, 1) {
            ...
            let lunar = LunarCalendarUtil.convertSolar2Lunar(dd: d, mm: nextMonth, yy: nextYear)
            let lunStr = lunar.lunarDay == 1 ? "\(lunar.lunarDay)/\(lunar.lunarMonth)" : "\(lunar.lunarDay)"
            let dow = dayOfWeekFor(day: d, month: nextMonth, year: nextYear)
            days.append(CalendarDay(
    ...
    private func calculateDayRating(truc: TrucNgayInfo, sao: SaoChieuInfo, activities: DayActivities) -> DayRatingInfo {
        ...
        let label: String
        switch score {
        case 80...100: label = "Rất tốt"
        case 60..<80: label = "Tốt"
        case 40..<60: label = "Trung bình"
        default: label = "Xấu"
        }
    ...
    private func dayOfWeekFor(day: Int, month: Int, year: Int) -> Int {
        // Returns 1=Sun..7=Sat
        let calendar = Calendar.current
        guard let date = calendar.date(from: DateComponents(year: year, month: month, day: day)) else { return 1 }
